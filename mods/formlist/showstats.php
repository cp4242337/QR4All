...
foreach ($forms as $cl) {
	foreach ($cl->cats as $ct) {
		if ($ct->forms) {
			...
			echo 'data'.$ct->cat_id.'.addColumn(\'string\',\'Form\');'."\n";
			echo 'data'.$ct->cat_id.'.addColumn(\'number\',\'Hits\');'."\n";
			echo 'data'.$ct->cat_id.'.addRows('.count($ct->forms).');'."\n";
			$row=0;
			$hitstotal=0;
			$pertot=0;
			foreach ($ct->forms as $cd) {
				echo 'data'.$ct->cat_id.'.setValue('.$row.',0,\''.$cd->form_title.'\');'."\n";
			...
			foreach ($ct->forms as $d) { 
				echo '<tr>';
				echo '<td>'.$d->form_title.'&nbsp;</td>';
			...
			foreach ($ct->forms as $cd) {
				foreach ($cd->coords as $h) {
					if ($h->hit_lat) {
						...
						echo 'var marker'.$count.' = new google.maps.Marker({position: latlong'.$count.', map: map, title:"'.$cd->form_title.': '.$h->hits.'"});'."\n";
					}
					$count++;
				}
			}
