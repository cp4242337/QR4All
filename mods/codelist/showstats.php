		...
		foreach ($codes as $cl) {
			foreach ($cl->cats as $ct) {
				foreach ($ct->codes as $cd) {
					echo 'data.addColumn(\'number\',\''.$cd->cd_name.'\');'."\n";
					$clcount++;
				}
			}
		}    
...
foreach ($codes as $cl) {
	foreach ($cl->cats as $ct) {
		if ($ct->codes) {
			...
			echo 'data'.$ct->cat_id.'.addColumn(\'string\',\'Code\');'."\n";
			echo 'data'.$ct->cat_id.'.addColumn(\'number\',\'Hits\');'."\n";
			echo 'data'.$ct->cat_id.'.addRows('.count($ct->codes).');'."\n";
			$row=0;
			$hitstotal=0;
			$pertot=0;
			foreach ($ct->codes as $cd) {
				echo 'data'.$ct->cat_id.'.setValue('.$row.',0,\''.$cd->cd_name.'\');'."\n";
			...
			foreach ($ct->codes as $d) { 
				echo '<tr>';
				echo '<td>'.$d->cd_name.'&nbsp;</td>';
			...
			echo '<tr><td colspan="2" align="right"><b>Total:</b></td><td align="right">'.$hitstotal.'</td><td align="right">'.$pertot.'%</td></tr>';
			echo '</table></div>';
			echo '</div>';
			echo '</div>';
			foreach ($ct->codes as $o) {
...
echo 'datab.addColumn(\'string\',\'Code\');'."\n";
...
echo 'datap.addColumn(\'string\',\'Code\');'."\n";
...
echo 'datam.addColumn(\'string\',\'Code\');'."\n";
...
echo 'datac.addColumn(\'string\',\'Code\');'."\n";
...
echo 'dataz.addColumn(\'string\',\'Code\');'."\n";
			...
			foreach ($ct->codes as $cd) {
				foreach ($cd->coords as $h) {
					if ($h->hit_lat) {
						...
						echo 'var marker'.$count.' = new google.maps.Marker({position: latlong'.$count.', map: map, title:"'.$cd->cd_name.': '.$h->hits.'"});'."\n";
					}
					$count++;
				}
			}
